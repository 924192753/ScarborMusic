...
| Web 重启、`Cannot find module 'effect'` | 需含 `prisma-cli` 阶段的 Dockerfile，`git pull` 后重建 web |
| `Cannot find module 'dotenv/config'` | 同上；或设置 `NODE_PATH` + 复制 dotenv 的最新镜像 |
...
# ── 2. 解压或 git clone ──
cd /opt/scarbormusic   # 项目根（含 docker-compose.prod.yml）

# ── 3. 环境变量 ──
chmod +x scripts/*.sh
cp .env.production.ip.example .env.production
# 替换 change-me；或: ./scripts/setup-env.sh
vim .env.production
grep change-me .env.production || echo "env OK"

# ── 4. 拉最新代码（重要：含 Prisma effect 修复）──
git pull
...
# ── 6. 验证 ──
docker logs scarbormusic-web --tail 30    # 应看到 migrate 成功 + Starting Next.js
./scripts/health-check.sh
curl -I http://159.75.87.182/
```

**访问：** http://159.75.87.182 · Grafana: http://159.75.87.182:3001
